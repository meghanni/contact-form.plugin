...
class cff_Contact
{
    ...
    function IsValid() 
    {
        ...
        if ($this->Email != $this->ConfirmEmail)
            $this->Errors['Confirm-Email']='Sorry the email addresses do not match.';

        //email
        
        if (strlen($this->Email) == 0) 
            $this->Errors['Email'] = "Please give your email address.";
        
        //email
        
        if (strlen($this->ConfirmEmail) == 0) 
            $this->Errors['Confirm-Email'] = "Please confirm your email address.";        
        ...
        if ( $this->RecaptchaPublicKey <> '' && $this->RecaptchaPrivateKey <> '' ) 
        {
            ...
            if ( ! $resp->is_valid ) 
                $this->Errors['recaptcha'] = "Sorry the code wasn't entered correctly please try again.";
        }
